...
const enableShare = async (req: myRequest, res: Response) => {
    ...
    try {
        ...
        const updateContent = await contentModel.updateOne({ contentBy: userId, _id: contentId }, { $set: { canShared: true, shareLink: `${sharedId}` } });
        const UpdatedContent = await contentModel.findOne({ contentBy: userId, _id: contentId });

        if (updateContent) {
            if (UpdatedContent) {
                res.status(200).json({ "message": "Now You Can share Your Post", "link": `${SHARED_URL}/${UpdatedContent.shareLink}` });
            }
        } else {
